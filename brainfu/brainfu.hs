...
run code input = go ([],code) ([],repeat 0) input where
  ...
  go (cl,c:cr) (ml,m:mr) i = case c of
    ...
    '[' | m==0
        -> let (l,r) = goto (c:cl,cr) 1 in go (l,r) (ml,m:mr) i
    ']' | m/=0
        -> let (r,l) = goto (c:cr,cl) (-1) in go (l,r) (ml,m:mr) i
    _   -> go (c:cl,cr) (ml,m:mr) i

  goto :: (String,String) -> Int -> (String,String)
  goto (l,c:r) n = if n == 0 then (l,c:r)
                   else goto (c:l,r) (case c of '[' -> n+1 ; ']' -> n-1 ; _ -> n)
